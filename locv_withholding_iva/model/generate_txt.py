...
class TxtIva(models.Model):
    ...
    def get_amount_line(self, txt_line, amount_exempt):
        ...
        exempt = 0
        id_tax = txt_line.tax_wh_iva_id.id_tax
        alic_porc = 0
        if id_tax:
            busq = self.env['account.tax'].search([('id', '=', id_tax)])
            if busq:
